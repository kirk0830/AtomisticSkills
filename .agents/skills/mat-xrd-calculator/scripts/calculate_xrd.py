...
def calculate_xrd(
    ...
    sim_xrd, theta = get_sim_xrd_from_pattern(
        pattern, eta, caglioti_params, bin=bin
    )  # simulates
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Calculate XRD spectrum using pymatgen."
    )
    parser.add_argument(
        "structure", help="Path to input structure file (CIF, POSCAR, etc.)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--wavelength",
        default="CuKa",
        help="Radiation wavelength or source name (default: CuKa).",
    )
    parser.add_argument(
        "--symprec",
        type=float,
        default=0.1,
        help="Symmetry precision for XRD calculation.",
    )
    parser.add_argument(
        "--eta",
        type=float,
        default=0.1,
        help="Fraction of Lorentzian component for simulated XRD.",
    )
    parser.add_argument(
        "--caglioti_params",
        type=tuple,
        default=(0.1, 0.01, 0.1),
        help="Caglioti parameters for simulated XRD.",
    )
    parser.add_argument(
        "--bin", type=float, default=0.05, help="Bin size for simulated XRD."
    )
