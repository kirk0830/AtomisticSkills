...
class CrystalFeatureObserver:
    ...
    def __call__(self) -> None:
        ...
        if hasattr(self.atoms, "calc") and self.atoms.calc is not None:
            ...
            if (
                hasattr(self.atoms.calc, "results")
                and "crystal_fea" in self.atoms.calc.results
            ):
...
class OffEquilibriumSampler:
    ...
    def __init__(
        ...
        expanded_atoms = expand_structure(
            atoms,
            target_atoms=target_atoms,
            max_atoms=MAX_SAFE_ATOMS,
        ...
        logger.info(
            "Structure relaxed after supercell expansion to remove residual stress"
        )
        ...
        if time_step is not None:
            ...
            if "H" in symbols:
                ...
                logger.info(
                    f"No Hydrogen detected, using {self.time_step} fs time step"
                )
    ...
    def _relax_structure(self, atoms: Atoms) -> Atoms:
        ...
        logger.info("Relaxing structure before supercell expansion")
        ...
        if isinstance(result, dict):
            if "structure" in result:
                ...
                raise ValueError(
                    f"Could not find relaxed structure in result. Result keys: {list(result.keys())}"
                )
        ...
        energy = (
            result.get("energy", "N/A")
            if isinstance(result, dict)
            else getattr(result, "energy", "N/A")
        )
    ...
    def run_md_simulation(self) -> Tuple[List[Atoms], List[np.ndarray]]:
        ...
        self.trajectory_observer = TrajectoryObserver(
            atoms, log_interval=self.log_interval
        )
        self.crystal_feature_observer = CrystalFeatureObserver(
            atoms, log_interval=self.log_interval
        )
        ...
        logger.info(
            f"Starting MD from {initial_temp}K, will gradually heat to {self.temperature}K via thermostat"
        )
        ...
        if Inhomogeneous_NPTBerendsen is None:
            raise RuntimeError(
                "Inhomogeneous_NPTBerendsen not available. Please ensure ASE is properly installed."
            )
        ...
        if self.ensemble == "npt":
            ...
            raise ValueError(
                f"Unsupported ensemble '{self.ensemble}'. Must be 'nvt' or 'npt'."
            )
        ...
        logger_md = MDLogger(
            dyn, atoms, str(log_path), header=True, stress=True, peratom=True, mode="w"
        )
        ...
        return (
            self.trajectory_observer.structures,
            self.crystal_feature_observer.crystal_feature_vectors,
            {
                "num_atoms": len(atoms),
    ...
    def cluster_structures(
        self, structures: List[Atoms], features: List[np.ndarray]
        ...
        logger.info(
            f"Clustering {len(structures)} structures to {min(self.n_clusters, len(structures))} representatives"
        )
        ...
        if n_clusters >= len(structures):
            logger.info(
                "Number of clusters >= number of structures, returning all structures"
            )
        ...
        if len(structures) < 10:
            ...
            kmeans = KMeans(
                n_clusters=n_clusters, random_state=42, n_init=10, max_iter=100
            )
            cluster_labels = kmeans.fit_predict(features_scaled)

        # Select representative structures
        representative_structures, representative_indices = (
        ...
        logger.info(
            f"Clustering completed. Selected {len(representative_structures)} representative structures"
        )
    ...
    def _simple_selection(
        self, structures: List[Atoms], features: np.ndarray, n_clusters: int
    ) -> List[Atoms]:
        """
    ...
    def _select_representatives(
        ...
        for cluster_id in range(n_clusters):
            ...
            cluster_structures = [
                structures[i] for i in range(len(structures)) if cluster_mask[i]
            ]
            ...
            if len(cluster_structures) > 0:
                if len(cluster_structures) == 1:
                    ...
                    distances = np.linalg.norm(
                        cluster_features - cluster_centroid, axis=1
                    )
    ...
    def sample(self) -> Tuple[List[Atoms], Dict[str, Any]]:
        ...
        if len(features) == 0:
            logger.warning(
                "No crystal features extracted. Improving fall-back to uniform temporal sampling."
            )
            # Fallback to simple uniform sampling
            n_samples = min(self.n_clusters, len(structures))
            if n_samples > 0:
                indices = np.linspace(
                    0, len(structures) - 1, n_samples, dtype=int
                ).tolist()
            ...
            representative_structures, representative_indices = self.cluster_structures(
                structures, features
            )
        ...
        metadata["sampled_indices"] = (
            representative_indices  # Index in the structure list
        )
        ...
        if self.output_dir:
            try:
                ...
                for i, struct in enumerate(representative_structures):
                    ...
                    write(fname, struct)
                logger.info(
                    f"Saved {len(representative_structures)} structures to {self.output_dir}"
                )
            except Exception as e:
                logger.warning(f"Failed to save sampled structures: {e}")

        logger.info(
            f"PES sampling completed. Generated {len(representative_structures)} representative structures"
        )
