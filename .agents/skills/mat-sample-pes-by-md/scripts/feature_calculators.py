...
class MatGLCrystalFeaturePotential(nn.Module):
    ...
    def forward(
        ...
        batch_size = (
            ...
            else (g.num_graphs if hasattr(g, "num_graphs") else 1)
        )
        ...
        if not isinstance(model_output, dict):
            raise TypeError(
                f"Model {self.model.__class__.__name__} must return a dict when return_all_layer_output=True, instead got {type(model_output)}"
            )
        ...
        if total_energy is None:
            raise KeyError(
                f"Model output dict did not contain 'final' energy key. Available keys: {list(model_output.keys())}"
            )
        ...
        if crystal_features is None:
            raise KeyError(
                f"Could not extract crystal features from model output keys: {list(model_output.keys())}"
            )
...
class MatGLCrystalFeatureCalculator(Calculator):
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        if self.converter is None:
            ...
            elements = (
                self.potential.model.element_types
                if hasattr(self.potential.model, "element_types")
        ...
        state_attr = (
            torch.tensor(
                state_attr, dtype=matgl.float_th, device=self.device
            ).unsqueeze(0)
        ...
        # Run model
        energy, forces, stress, hessian, crystal_features = self.potential(
            g_batch, lattice, state_attr
        )
...
class MaceCrystalFeatureCalculator(Calculator):
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        try:
            ...
            self.mace_calc.calculate(
                atoms, properties=req_props, system_changes=system_changes
            )
            ...
            if captured_feats:
                ...
                if descriptors is not None:
                    ...
                    raise RuntimeError(
                        "MACE descriptor extraction failed (both hook and fallback)"
                    )
        finally:
            # Always remove hooks
            for h in hooks:
