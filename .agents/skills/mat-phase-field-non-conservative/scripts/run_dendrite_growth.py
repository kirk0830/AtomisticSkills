...
def main():
    parser = argparse.ArgumentParser(
        description="Simulate Dendrite Growth (Kobayashi 1993)"
    )
    parser.add_argument(
        "--grid-size", type=int, default=300, help="Number of cells in x and y"
    )
    ...
    parser.add_argument(
        "--output",
        type=str,
        default=".agents/skills/mat-phase-field-allen-cahn/examples/benchmark-dendrite/dendrite.gif",
        help="Output gif or png path",
    )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        for step in tqdm(range(args.steps)):
            ...
            dphi_dt = (
                term1_x + term2_y + laplacian_term + phi * (1.0 - phi) * (phi - 0.5 + m)
            ) / tau
