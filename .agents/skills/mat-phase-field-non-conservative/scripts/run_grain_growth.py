...
def main():
    parser = argparse.ArgumentParser(
        description="Simulate 2D Curvature-driven Grain Shrinkage using Allen-Cahn equation in FiPy"
    )
    parser.add_argument(
        "--grid-size", type=int, default=50, help="Number of cells in x and y"
    )
    parser.add_argument(
        "--radius", type=float, default=15.0, help="Initial radius of the grain"
    )
    ...
    parser.add_argument(
        "--output", type=str, default="grain_growth.gif", help="Output gif or png path"
    )
    ...
    plt.grid(True)
    area_out = args.output.replace(".gif", "_area_decay.png").replace(
        ".png", "_area_decay.png"
    )
