...
def load_relaxed_solids(
    relaxed_dir: Path, exclude_formulas: List[str] = None
) -> Dict[str, Dict]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Strict Pymatgen Pourbaix Diagram Script"
    )
    parser.add_argument(
        "--relaxed_solids",
        type=Path,
        required=True,
        help="Dir containing relaxed solid results",
    )
    parser.add_argument(
        "--target",
        type=str,
        help="Target metal element (required if comp_dict not provided)",
    )
    parser.add_argument("--output", type=Path, required=True, help="Output directory")
    parser.add_argument(
        "--ion_concentration", type=float, default=1e-6, help="Ion concentration in M"
    )
    parser.add_argument(
        "--exclude_phases", nargs="*", help="Formulas to exclude (e.g. Zn(HO)2)"
    )
    parser.add_argument(
        "--comp_dict", type=str, help='Composition dictionary, e.g. "Li=1,Fe=1"'
    )
    parser.add_argument(
        "--conc_dict", type=str, help='Concentration dictionary, e.g. "Li=1e-6,Fe=1e-6"'
    )
    parser.add_argument(
        "--mlip_name", type=str, default="MLIP", help="MLIP name for plot title"
    )
    parser.add_argument(
        "--mlip_head",
        type=str,
        default=None,
        help="MLIP head name (optional, for compatibility check)",
    )
    parser.add_argument(
        "--apply_solid_compat",
        action="store_true",
        help="Apply MP2020 solid compatibility (for MP-trained MLIPs)",
    )
    ...
    if not args.apply_solid_compat:
        try:
            ...
            if yaml_path.exists():
                ...
                checkpoints = compat_data.get(
                    "checkpoints_requiring_mp2020_compatibility", []
                )
                    ...
                    if ckpt.get("name") == args.mlip_name:
                        ...
                        if config_head:
                            if args.mlip_head == config_head:
                                ...
                                print(
                                    f"Auto-detected need for MP2020 compatibility for {args.mlip_name} (head: {args.mlip_head})"
                                )
                                break
                        else:
                            ...
                            args.apply_solid_compat = True
                            print(
                                f"Auto-detected need for MP2020 compatibility for {args.mlip_name}"
                            )
                            break
    ...
    ee_path = (
        ...
        / "elemental-energies"
        / "resources"
        / f"{args.mlip_name}_energies.json"
    )

    if not ee_path.exists():
        print(
            f"Warning: {ee_path} not found. Ensure elemental-energies skill is run for {args.mlip_name}."
        )
        ...
        if h2o_json_path.exists():
            ...
            for k in keys_to_try:
                if k in h2o_data:
                    ...
                    print(
                        f"Loaded H2O energy for {k} from JSON: {h2o_energy_per_atom:.4f} eV/atom"
                    )
                    break
    except Exception as e:
    ...
    if h2o_energy_per_atom is None:
        ...
        if not h2o_candidates:
            print(
                "Error: No H2O relaxation found in relaxed_solids. Required for aqueous compatibility (and not found in pre-calculated JSON)."
            )
            return 1
        ...
        for p in h2o_candidates:
            if (p / "relaxed_energy.txt").exists() and (
                p / "relaxed_structure.cif"
            ).exists():
    ...
    print("Setting up MaterialsProjectAqueousCompatibility...")
    print(f"  O2 energy: {mu_O:.4f} eV/atom")
    print(f"  H2O energy: {h2o_energy_per_atom:.4f} eV/atom")

    # Disable POTCAR checks for MLIP data
    solid_compat = (
        MaterialsProject2020Compatibility(check_potcar=False, check_potcar_hash=False)
        if args.apply_solid_compat
        else None
    )
    ...
    for eid, data in mlip_solids_data.items():
        ...
        entry = ComputedEntry(
            data["composition"], data["energy"], entry_id=eid, parameters=parameters
        )
    ...
    for el in all_elements:
        ...
        for entry in computed_entries:
            if (
                ...
                and entry.composition.elements[0] == el
            ):
                has_element = True
                break
        ...
        if not has_element:
            try:
                ...
                print(
                    f"Auto-adding missing terminal entry: {label} (E={total_energy:.4f} eV)"
                )
                ...
                computed_entries.append(
                    ComputedEntry(
                        label,
                        total_energy,
                        entry_id=f"{label}_ref_ee",
                        parameters={"run_type": "GGA"},
                    )
                )
            except ValueError:
                print(
                    f"Warning: Missing terminal entry for {el} and could not find reference energy. Phase Diagram construction may fail."
                )
    ...
    for entry in processed_entries:
        ...
        n_non_oh = (
            ...
            - entry.composition.get("H", 0)
            - entry.composition.get("O", 0)
        )
        if n_non_oh < 1e-3:
            continue
