...
def main():
    parser = argparse.ArgumentParser(
        description="Pure Materials Project Pourbaix Diagram Script"
    )
    parser.add_argument(
        "--comp_dict",
        type=str,
        required=True,
        help='Composition dictionary, e.g. "Li=1,Fe=1"',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-6,
        help="Default ion concentration in M",
    )
    parser.add_argument(
        "--title", type=str, default="Materials Project Pourbaix", help="Plot title"
    )
    ...
    try:
        pb = PourbaixDiagram(
            entries, comp_dict=comp_dict, conc_dict=conc_dict, filter_solids=True
        )
    ...
    # Use standard limits
    ax = plotter.get_pourbaix_plot(limits=[[-2, 16], [-4, 4]], label_domains=True)
