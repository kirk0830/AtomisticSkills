...
def get_structures(chemsys: str, api_key: str, output_dir: Path):
    ...
    with MPRester(api_key) as mpr:
        ...
        solid_ids = [
            e.entry_id.split("-")[0] + "-" + e.entry_id.split("-")[1] for e in solids
        ]
        # Better robust way:
        clean_ids = []
        ...
        for doc in docs:
            ...
            if (
                formula not in best_docs
                or doc.energy_per_atom < best_docs[formula].energy_per_atom
            ):
                best_docs[formula] = doc

        print(
            f"Filtered to {len(best_docs)} lowest energy polymorphs (from {len(docs)} retrieved)."
        )
        ...
        for el in elements:
                ...
                if ref_file.exists():
                    ...
                    if h2o_src.exists():
                        ...
                        if not h2o_dst.exists():
                            ...
                            print("  Copied local reference: H2O.cif")
                    continue
            ...
            if docs:
                ...
                print(
                    f"  Saved reference for {el}: {filename.name} (Energy: {ref.energy_per_atom:.4f} eV/atom)"
                )
            else:
        ...
        print("2. Run calculate_formation_energies.py")
