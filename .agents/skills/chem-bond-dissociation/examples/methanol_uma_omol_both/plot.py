...
ax.set_title(
    "Methanol: Homolytic vs Heterolytic BDE\n(FairChem UMA uma-s-1p1, omol task)",
    fontweight="bold",
)
# Custom legend
...
co_bond = next(
    (
        b
        for b in bonds
        if "C" in b["bond_type"] and "O" in b["bond_type"] and b["hetero"] is not None
    ),
    None,
)
...
if co_bond:
    categories = [
        "Neutral radical\n(homolytic)",
        "Best ionic\n(heterolytic)",
        f"Variant A\n{co_bond['variant_A_lbl']}",
        f"Variant B\n{co_bond['variant_B_lbl']}",
    ]
    vals = [
        co_bond["homo"],
        co_bond["hetero"],
        co_bond["variant_A_val"],
        co_bond["variant_B_val"],
    ]
    ...
    best_idx = (
        2 if co_bond["heterolytic_best_variant"] == co_bond["variant_A_lbl"] else 3
    )
    ...
    ax2.set_title(
        f"C–O Bond: All Cleavage Modes\n({co_bond['frag1_formula']} / {co_bond['frag2_formula']})",
        fontweight="bold",
    )
    ...
    ax2.text(
        0.97,
        0.03,
        "★ gold border = best heterolytic variant",
