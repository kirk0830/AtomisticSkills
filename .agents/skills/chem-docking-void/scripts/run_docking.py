...
def prepare_ligand(smiles: str, num_conformers: int, seed: int = 42):
    ...
    if not cids:
        ...
        cids = AllChem.EmbedMultipleConfs(
            mol, numConfs=num_conformers * 5, useRandomCoords=True, randomSeed=seed
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Dock a small molecule into a porous material using VOID."
    )
    parser.add_argument(
        "--smiles", type=str, required=True, help="SMILES string of the guest ligand."
    )
    parser.add_argument(
        "--host_cif",
        type=str,
        required=True,
        help="Path to the CIF file for the host material.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="docking_output",
        help="Directory to save docked complexes (CIF format).",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of lowest energy conformers to test for docking.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.8,
        help="MinDistanceFitness threshold for complex.",
    )
    parser.add_argument(
        "--attempts", type=int, default=50, help="Number of attempts for Subdocker."
    )
    parser.add_argument(
        "--structs_per_loading",
        type=int,
        default=2,
        help="Max structures to keep per molecule loading.",
    )
    parser.add_argument(
        "--num_clusters",
        type=int,
        default=5,
        help="Number of clusters for Voronoi sampler.",
    )
    parser.add_argument(
        "--min_radius",
        type=float,
        default=3.0,
        help="Minimum radius for Voronoi sampler.",
    )
    parser.add_argument(
        "--probe_radius",
        type=float,
        default=0.1,
        help="Probe radius for Voronoi sampler.",
    )
    parser.add_argument(
        "--remove_species",
        type=str,
        nargs="*",
        default=[],
        help="Species to remove from host before docking.",
    )
    parser.add_argument(
        "--max_loading",
        type=int,
        default=20,
        help="Max loading of guest molecules per host.",
    )
    parser.add_argument(
        "--max_subdock",
        type=int,
        default=1,
        help="Max structures generated per docking run.",
    )
    ...
    # Prepare Ligand
    print(
        f"Preparing up to {args.num_conformers} lowest-energy 3D conformers for {args.smiles}..."
    )
    ...
    with open(os.path.join(args.output_dir, "docking_results.json"), "w") as f:
        json.dump(
            {"hyperparameters": vars(args), "results": results_metadata}, f, indent=4
        )
