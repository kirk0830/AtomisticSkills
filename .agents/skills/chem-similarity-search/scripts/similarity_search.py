...
def query_fastsimilarity_2d(
    identifier, namespace="smiles", threshold=90, max_records=10
):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.code in [503, 504]:
                ...
                print(
                    f"PubChem server busy (HTTP {e.code}). Retrying in {wait_time}s..."
                )
                ...
                print(
                    f"HTTP 400: Bad request. Possibly invalid SMILES or CID: {identifier}."
                )
                return None
            else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Query PubChem for fast 2D chemical similarity."
    )
    ...
    parser.add_argument(
        "--output", default="similarity_results.json", help="Output filename"
    )
    ...
    data = query_fastsimilarity_2d(
        identifier, namespace, args.threshold, args.max_records
    )
