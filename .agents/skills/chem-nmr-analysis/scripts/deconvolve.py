...
def main():
    ...
    _P(args.plot).parent if args.plot else _P('.').mkdir(parents=True, exist_ok=True)
    with open(_P(args.plot).parent if args.plot else _P('.') / "input_configs.yaml", 'w') as _f:
