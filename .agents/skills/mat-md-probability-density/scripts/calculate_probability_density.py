...
def calculate_probability_density(
    ...
    print(
        f"Extracting fractional coordinates from {len(atoms_list) - n_skip} frames..."
    )
    ...
    framework_indices = [
        atom.index for atom in atoms_list[n_skip] if atom.symbol != species
    ]
    ...
    trajectories = np.array(
        [atoms.get_scaled_positions() for atoms in atoms_list[n_skip:]]
    )
    ...
    if log_compression:
        print(
            "Applying logarithmic compression to enhance visualize continuous paths..."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Grid interval in Angstroms (default: 0.2)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--log",
        action="store_true",
        help="Apply logarithmic compression to enhance saddle points (best for <50ps MD)",
    )
    parser.add_argument(
        "--output_chgcar",
        default="CHGCAR_proba",
        help="Output path for the CHGCAR file",
    )
