...
try:
    ...
    print(
        "Please install it in this environment by running:\n    pip install rxn4chemistry"
    )
    sys.exit(1)
...
def evaluate_retrosynthesis(smiles: str, max_steps: int = 3):
    ...
    try:
        response = wrapper.predict_automatic_retrosynthesis(
            smiles=smiles, max_steps=max_steps
        )
        job_id = response["prediction_id"]
        print(f"Job successfully submitted! (ID: {job_id})")
        print(
            "Polling for completion... (This may take a few minutes for complex molecules)"
        )
    ...
    while True:
        try:
            ...
            if status == "SUCCESS":
                ...
                print(
                    "The retrosynthesis job encountered an error on the IBM RXN server."
                )
                sys.exit(1)
    ...
    def print_tree(node, depth=0):
        ...
        is_commercially_available = (
            "📦 (commercial)" if node.get("is_commercial", False) else ""
        )
    ...
    print("-" * 80)
    print(
        "For full interactive visualization, check the project in your IBM RXN dashboard."
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate synthetic accessibility using IBM RXN"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Maximum number of synthesis steps to explore",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=".",
        help="Directory to save input_configs.yaml",
    )
