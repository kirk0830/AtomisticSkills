...
def create_interface(solid_path, liquid_path, axis, output_path):
    ...
    # axis 0=x, 1=y, 2=z
    combined = stack(
        solid, liquid, axis=axis, distance=2.0, maxstrain=2.0
    )  # Allow up to 200% strain for melting
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a solid-liquid interface by stacking two structures."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Axis along which to stack (0=x, 1=y, 2=z, default: 2)",
    )
    parser.add_argument(
        "--output",
        default="interface.cif",
        help="Output file path (default: interface.cif)",
    )
