...
def main():
    ...
    parser.add_argument(
        "features_file", help="JSON file with atomic features to analyze"
    )
    parser.add_argument(
        "--solid_features",
        required=True,
        help="JSON file with reference solid atomic features",
    )
    parser.add_argument(
        "--liquid_features",
        required=True,
        help="JSON file with reference liquid atomic features",
    )
    parser.add_argument(
    ...
    if args.verbose:
        print(
            f"Loaded features: {descriptors.shape[0]} atoms, {descriptors.shape[1]} features"
        )
        print(f"Solid reference: {mu_solid.shape}")
        print(f"Liquid reference: {mu_liquid.shape}")

    # Classify
    is_solid, projections, solid_fraction = project_and_classify(
        descriptors, mu_solid, mu_liquid
    )
