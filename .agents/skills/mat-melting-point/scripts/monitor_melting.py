...
def monitor_melting(
    log_path,
    analysis_window_ps=1.0,
    stability_duration_ps=5.0,
    timestep_fs=1.0,
    stop_file="MD_STOP",
):
    """
    ...
    print(
        f"Stability Duration: {stability_duration_ps}ps (checking for flattened StdDev)"
    )
    ...
    while True:
        try:
            ...
            stats_history.append(
                {"time": current_time, "std_t": curr_std_t, "std_epot": curr_std_epot}
            )
            ...
            if span >= (stability_duration_ps * 0.9):
                ...
                if curr_std_t < 50.0:
                    print(
                        f"STABILITY REACHED at {current_time:.2f}ps (Temp StdDev {curr_std_t:.2f} < 50)"
                    )
                    ...
                    if pid:
                        ...
                        if pid != os.getpid():
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    print(
                                        f"Process {pid} still alive, sending SIGKILL..."
                                    )
                                    os.kill(pid, 9)
                                except OSError:
                                    pass  # Process dead
                        ...
                        print(
                            f"Could not find PID file at {pid_file}. Attempting lsof fallback..."
                        )
                        try:
                            ...
                            print(
                                f"Fallback kill failed: {k_err}. Please stop manually."
                            )
                    return
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--window", type=float, default=1.0, help="Window to calc local StdDev (ps)"
    )
    parser.add_argument(
        "--stability_duration",
        type=float,
        default=5.0,
        help="Duration StdDev must be flat (ps)",
    )
    ...
    monitor_melting(
        args.log_path,
        analysis_window_ps=args.window,
        stability_duration_ps=args.stability_duration,
