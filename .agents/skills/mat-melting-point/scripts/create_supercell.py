...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a rectangular supercell for melting point simulations."
    )
    ...
    parser.add_argument(
        "--min_length", type=float, help="Minimum length for all sides (default 20.0 A)"
    )
    parser.add_argument(
        "--min_lengths", type=float, nargs=3, help="Minimum length for [Lx, Ly, Lz]"
    )
    parser.add_argument(
        "--scaling", type=int, nargs=3, help="Explicit scaling factors [nx, ny, nz]"
    )
    ...
    target_min = (
        args.min_lengths
        if args.min_lengths
        else (args.min_length if args.min_length else 20.0)
    )
