...
def plot_arrhenius_custom(
    ...
    plt.plot(
        ...
        "r-",
        linewidth=2,
        label=f"$E_a = {Ea:.3f} \pm {std_Ea:.3f}$ eV",
    )
...
def calculate_activation_energy(root_dir: str = ".", struct_path: str = None) -> None:
    ...
    if r2 < 0.95:
        ...
        print(
            "This could be due to phase transitions, melting, non-linear diffusion regimes, or isolated outliers."
        )
    ...
    sigma_lnD_RT = get_extrapolation_uncertainty(
        temps_arr, diffs_arr, RT, diff_errs_arr if has_err else None
    )
    ...
    if struct_path and os.path.exists(struct_path):
        ...
        sigma_RT = get_extrapolated_conductivity(
            temps_arr, diffs_arr, RT, structure, "Li"
        )
        sigma_RT_err = sigma_RT * sigma_lnD_RT
        print(
            f"Extrapolated RT (300K) Conductivity: {sigma_RT:.3f} +/- {sigma_RT_err:.3f} mS/cm"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to structural CIF for conductivity computation",
    )
