...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--limit", type=int, default=10, help="Maximum number of patents to retrieve"
    )
    parser.add_argument(
        "--output", type=str, default=None, help="Optional path to save results as JSON"
    )
    ...
    if results:
        for i, p in enumerate(results, 1):
            print(
                f"[{i}] Patent #: {p.get('publication_number')} | Priority Date: {p.get('priority_date')}"
            )
            print(f"    Title:    {p.get('title')}")
            print(f"    Assignee: {p.get('assignee')}")
            # Clean up snippet: remove bold HTML tags Google sometimes returns
            snippet = (
                html.unescape(str(p.get("snippet", "N/A")))
                .replace("<b>", "")
                .replace("</b>", "")
            )
    ...
    if args.output:
        with open(args.output, "w") as f:
            json.dump(
                {"query": args.query, "limit": args.limit, "results": results},
                f,
                indent=2,
            )
