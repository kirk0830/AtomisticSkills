...
def main():
    parser = argparse.ArgumentParser(
        description="Match target structure(s) against candidates."
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="MP",
        help="Path to candidate structure file/dir, or 'MP' to automatically query Materials Project.",
    )
    parser.add_argument(
        "--ltol", type=float, default=0.2, help="Fractional length tolerance."
    )
    ...
    parser.add_argument(
        "--output", type=str, default="match_results.json", help="Output JSON file."
    )
    ...
    if args.candidates.upper() == "MP":
        ...
        print(
            f"Querying Materials Project for {len(unique_formulas)} unique formulas..."
        )
        with MPRester() as mpr:
            for formula in unique_formulas:
                try:
                    docs = mpr.materials.summary.search(
                        formula=formula, fields=["structure", "material_id"]
                    )
                    structs = []
                    for doc in docs:
                        s = doc.structure
                    ...
                    print(
                        f"  -> Found {len(structs)} known MP polymorphs for {formula}."
                    )
                except Exception as e:
    ...
    for target in target_structs:
        ...
        if not result["match_found"] and len(target_structs) == 1 and has_lit_utils:
            if sg_symbol and crystal_system:
                ...
                print(
                    f"  -> Checking literature for '{formula}' with symmetry '{sg_symbol}'..."
                )
            else:
                query_str = f'"{formula}"'
            ...
            try:
                ...
                if lit_results:
                    ...
                    for i, r in enumerate(lit_results, 1):
                        ...
                        print(f"     [{i}] {title} ({doi})")
                        result["literature_matches"].append(
                            {"title": title, "doi": doi}
                        )
                else:
