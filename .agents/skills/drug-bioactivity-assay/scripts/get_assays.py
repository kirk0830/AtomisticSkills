...
def main():
    parser = argparse.ArgumentParser(
        description="Query PubChem for molecular bioactivity and target assays."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--active_only",
        action="store_true",
        help="Only return assays where the compound was 'Active'",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=1000,
        help="Maximum number of assay records to save (default: 1000)",
    )
    parser.add_argument("--outdir", required=True, help="Directory to save the results")
    parser.add_argument(
        "--output", default="assay_summary.json", help="Output filename"
    )
    ...
    for i, r in enumerate(records[:5]):
        ...
        print(
            f"[{i+1}] AID: {aid} | Outcome: {outcome} | Target GeneID: {gene} | Activity Val [uM]: {val}"
        )
