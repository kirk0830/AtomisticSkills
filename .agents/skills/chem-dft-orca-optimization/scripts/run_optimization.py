...
def run_optimization(
    ...
    with cwd(output_dir):
        if opt_type == "min":
            logger.info(
                f"Running geometry minimization (max {convergence_max_iterations} steps)..."
            )
            output_key = "opt"
            ...
            if calculate_final_hessian:
                systems, success = readuct.run_hessian_task(systems, [output_key])
        elif opt_type == "ts":
            logger.info(
                f"Running TS optimization (max {convergence_max_iterations} steps)..."
            )
            output_key = "tsopt"
        ...
        if success:
            ...
            logger.warning(
                "Optimization did NOT converge within the maximum number of steps."
            )
    ...
    if (
        final_results.energy is None
        or final_results.gradients is None
        or (calculate_final_hessian and final_results.hessian is None)
    ):
        if calculate_final_hessian:
            opt_calculator.set_required_properties(
                [su.Property.Energy, su.Property.Gradients, su.Property.Hessian]
            )
        else:
            opt_calculator.set_required_properties(
                [su.Property.Energy, su.Property.Gradients]
            )
    ...
    logger.info(
        f"Final energy: {final_energy_hartree:.10f} Hartree = {final_energy_ev:.6f} eV"
    )
    logger.info(
        f"Final max force: {max_force:.6f} eV/A, RMS force: {rms_force:.6f} eV/A"
    )
