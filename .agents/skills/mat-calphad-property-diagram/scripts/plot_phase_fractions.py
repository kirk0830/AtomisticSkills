...
def main():
    parser = argparse.ArgumentParser(
        description="Plot equilibrium phase fractions vs temperature for a specific alloy."
    )
    ...
    parser.add_argument(
        ...
        nargs=2,
        required=True,
        help="The two atomic elements (e.g., Al Zn).",
    )
    parser.add_argument(
        "--composition",
        nargs=2,
        required=True,
        help="Target solute element and its mole fraction (e.g., Zn 0.1).",
    )
    parser.add_argument(
        "--t-range",
        nargs=3,
    ...
    conds = {v.N: 1, v.P: 101325, v.T: (t_start, t_stop, t_step), v.X(solute): comp_val}
