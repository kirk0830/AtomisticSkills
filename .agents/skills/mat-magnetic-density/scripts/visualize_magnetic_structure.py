...
def visualize_magnetic_structure(
    structure_path: Path, moments_data: Dict[str, Any], output_path: Path
) -> None:
    """
    ...
    if len(site_moments) != len(structure):
        print(
            f"Warning: Number of moments ({len(site_moments)}) does not match "
...
def main():
    ...
    parser.add_argument(
        "structure_file", type=str, help="Path to structure file (CIF, POSCAR, etc.)"
    )
    parser.add_argument(
        "moments_file",
        type=str,
    ...
    if not structure_path.exists():
        print(
            f"Error: Structure file '{args.structure_file}' not found", file=sys.stderr
        )
        sys.exit(1)

    if not moments_path.exists():
        print(f"Error: Moments file '{args.moments_file}' not found", file=sys.stderr)
        sys.exit(1)
