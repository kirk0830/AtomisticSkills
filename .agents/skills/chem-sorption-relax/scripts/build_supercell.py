...
def main():
    parser = argparse.ArgumentParser(
        description="Build a supercell based on a minimum interplanar distance."
    )
    parser.add_argument(
        "--structure",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to save the generated supercell CIF",
    )
    ...
    if not all(atoms.pbc):
        print(
            "Warning: Input structure does not have PBC in all directions. Adding PBC."
        )
