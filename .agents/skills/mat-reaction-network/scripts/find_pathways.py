...
def main():
    ...
    parser.add_argument(
        "--target", required=True, help="Target material formula (e.g., 'YMnO3')"
    )
    parser.add_argument(
        "--precursors",
        nargs="+",
        required=True,
        help="List of precursor formulas (e.g., 'YCl3' 'Mn2O3' 'Li2CO3')",
    )
    parser.add_argument(
        "--byproducts",
        nargs="*",
        default=[],
        help="List of byproduct formulas to allow in the net reaction (e.g., 'LiCl' 'CO2')",
    )
    parser.add_argument(
        "--temperature",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of shortest paths to find (default: 5)",
    )
    parser.add_argument(
        "--paths-to-solve",
        type=int,
        default=4,
        help="Number of balanced pathway combinations to solve (default: 4)",
    )
    parser.add_argument(
        "--stability-tol",
        type=float,
        default=0.0,
        help="Energy above hull tolerance in eV/atom (default: 0.0 for stable only)",
    )
    parser.add_argument(
        "--output", type=str, help="Optional text file to save the output logs to."
    )
    ...
    if not paths or not paths.paths:
        print(
            "No pathways found. Try increasing --stability-tol or changing precursors."
        )
        return
    ...
    try:
        balanced_paths = ps.solve(
            net_rxn,
            max_num_combos=args.paths_to_solve,
    ...
    for p in paths:
        if len(p.reactions) == 1:
            rxn = p.reactions[0]
            if {e.reduced_formula for e in rxn.reactants} == {
                e.reduced_formula for e in net_rxn.reactants
            } and {e.reduced_formula for e in rxn.products} == {
                e.reduced_formula for e in net_rxn.products
            }:
    ...
    if args.output:
        ...
        for path in balanced_paths:
            path_dict = {
                "total_cost": getattr(
                    path, "average_cost", getattr(path, "cost", None)
                ),
                "steps": [],
            }
