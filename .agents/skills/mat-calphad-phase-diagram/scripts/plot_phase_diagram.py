...
def main():
    parser = argparse.ArgumentParser(
        description="Plot a binary phase diagram from a TDB database."
    )
    ...
    parser.add_argument(
        ...
        nargs=2,
        required=True,
        help="The two atomic elements to plot (e.g., Al Zn).",
    )
    parser.add_argument(
        "--t-range",
        nargs=3,
    ...
    parser.add_argument(
        ...
        help="Path to save the resulting plot (default: phase_diagram.png).",
    )
    ...
    logging.info(
        f"Generating Phase Diagram for {el1}-{el2} at P=1atm, T={t_start}-{t_stop} K..."
    )
