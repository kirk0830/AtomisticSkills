...
def run_thermal_conductivity(args, wrapper, atoms):
    ...
    if not args.output_dir:
        args.output_dir = str(
            get_current_research_dir() / "vibrational" / "lattice_thermal_conductivity"
        )
    ...
    summary = {
        "thermal_conductivity_summary": {
            "temp_300K": result["thermal_conductivity"][
                get_T_idx(result["temperatures"], 300)
            ]
            if "thermal_conductivity" in result
            else "N/A",
            "temp_100K": result["thermal_conductivity"][
                get_T_idx(result["temperatures"], 100)
            ]
            if "thermal_conductivity" in result
            else "N/A",
        },
    ...
    with open(
        os.path.join(args.output_dir, "lattice_thermal_conductivity_results.json"), "w"
    ) as f:
        json.dump(recursive_tolist(summary), f, indent=4)

    logger.info(
        f"Lattice thermal conductivity calculation completed. Results saved to {args.output_dir}"
    )
    return summary
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Calculate lattice thermal conductivity using MLIPs"
    )
    ...
    parser.add_argument(
        "--supercell_matrix",
        help="Supercell matrix for 2nd order force constant (JSON string, e.g. [[2,0,0],[0,2,0],[0,0,2]])",
    )
    parser.add_argument(
        "--supercell_matrix_3rd",
        help="Supercell matrix for 3rd order force constant (JSON string, e.g. [[2,0,0],[0,2,0],[0,0,2]])",
    )
    parser.add_argument(
        "--mesh_numbers",
        type=int,
        nargs=3,
        default=[5, 5, 5],
        help="Mesh numbers for 3rd order force constant",
    )
    parser.add_argument(
