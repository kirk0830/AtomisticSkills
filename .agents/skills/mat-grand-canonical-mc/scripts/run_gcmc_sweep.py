...
def run_gcmc_single_point(
    ...
    ensemble = Ensemble.from_cluster_expansion(
        ce, supercell_matrix=supercell_matrix, chemical_potentials=chem_pots
    )

    # Create sampler with flip moves (semigrand canonical)
    sampler = Sampler.from_ensemble(ensemble, temperature=temperature, step_type="flip")

    # Generate initial random occupancy
    num_sites = ensemble.processor.num_sites
    initial_occu = np.zeros(
        num_sites, dtype=int
    )  # Start with all zeros (first species)
...
def main():
    ...
    parser.add_argument(
        "--ce_file", type=str, required=True, help="Path to cluster expansion JSON file"
    )
    parser.add_argument(
        "--supercell",
        type=int,
    ...
    parser.add_argument(
        "--mu_min", type=float, required=True, help="Minimum chemical potential in eV"
    )
    parser.add_argument(
        "--mu_max", type=float, required=True, help="Maximum chemical potential in eV"
    )
    parser.add_argument(
        "--num_mu_points",
        type=int,
    ...
    for temp in args.temperatures:
        ...
        for mu in mu_values:
            ...
            if "error" not in result:
                print(
                    f"  ✓ Composition: {result['mean_composition']:.4f} ± {result['std_composition']:.4f}"
                )
                print(
                    f"  ✓ Energy: {result['mean_energy']:.4f} ± {result['std_energy']:.4f} eV/atom"
                )
    ...
    if num_success > 0:
        if num_success == num_total:
            print("✓ GCMC sweep completed successfully!")
        else:
            print("⚠ GCMC sweep completed with some failures.")
    else:
        print("✗ GCMC sweep failed. No successful simulations.")
