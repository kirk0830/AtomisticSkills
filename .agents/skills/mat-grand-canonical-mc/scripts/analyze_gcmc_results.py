...
def plot_mu_vs_composition(results: List[Dict], element: str, output_file: str):
    """
...
def plot_phase_diagram(results: List[Dict], element: str, output_file: str):
    """
    ...
    ax.set_ylabel(f"Composition x({element})", fontsize=14)
    ax.set_title(
        "Temperature-Composition Phase Diagram", fontsize=16, fontweight="bold"
    )
    ax.grid(True, alpha=0.3)
    ax.set_ylim(-0.05, 1.05)
...
def plot_contour_phase_diagram(results: List[Dict], element: str, output_file: str):
    """
    ...
    contourf = ax.contourf(
        T_grid, mu_grid, comp_grid, levels=levels, cmap="RdYlBu_r", extend="both"
    )
    ...
    ax.set_ylabel(
        f"Chemical Potential μ({element}) [eV]", fontsize=13, fontweight="bold"
    )
    ax.set_title(
        f"{element} Composition: T-μ Phase Diagram", fontsize=14, fontweight="bold"
    )
...
def plot_energy_vs_mu(results: List[Dict], element: str, output_file: str):
    """
...
def print_summary_statistics(data: Dict[str, Any]):
    ...
    print(
        f"Chemical potential range: {metadata['mu_range'][0]:.3f} to {metadata['mu_range'][1]:.3f} eV"
    )
    print(
        f"MC steps: {metadata['steps']} (equilibration: {metadata['equilibration_steps']})"
    )
...
def main():
    ...
    parser.add_argument(
        "--element", type=str, required=True, help="Element name (e.g., 'Ag')"
    )
    ...
    plot_mu_vs_composition(
        results, args.element, str(output_dir / "mu_vs_composition.png")
    )

    plot_phase_diagram(results, args.element, str(output_dir / "phase_diagram.png"))

    plot_energy_vs_mu(results, args.element, str(output_dir / "energy_vs_mu.png"))

    plot_contour_phase_diagram(
        results, args.element, str(output_dir / "contour_phase_diagram.png")
    )
