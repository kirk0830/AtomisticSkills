...
class OrderDisorderSampler:
    ...
    def __init__(
        ...
        target_atoms: int = 50,
        include_perturbation: int = 1,
        perturbation_length: float = 0.1,
    ):
        """
    ...
    def _perturb_structure(self, atoms: Atoms, perturbation_length: float) -> Atoms:
        ...
        displacements = np.random.normal(
            0, perturbation_length / 3.0, size=perturbed.positions.shape
        )
        ...
        if max_disp > 0:
            displacements = (
                displacements * (perturbation_length / max_disp) * np.random.random()
            )
    ...
    def sample(self, output_dir: Optional[str] = None) -> List[Atoms]:
        ...
        """
        logger.info(
            f"Starting order-disorder sampling: generating {self.n_structures} ordered structures"
        )
        ...
        logger.info(
            f"Step 1: Expanding supercell to target size ({self.target_atoms} atoms)"
        )
        ...
        for nx in range(1, max_expansion + 1):
            for ny in range(1, max_expansion + 1):
                for nz in range(1, max_expansion + 1):
                    ...
                    cubicness = (
                        (max(nx, ny, nz) - min(nx, ny, nz)) / max(nx, ny, nz)
                        if max(nx, ny, nz) > 0
                        else 0.0
                    )
        ...
        logger.info(
            "Step 2: Performing OrderDisorderedStructureTransformation to generate candidates"
        )
        ...
        try:
            ...
            transformation = OrderDisorderedStructureTransformation(
                algo=EwaldMinimizer.ALGO_FAST, no_oxi_states=True
            )

            n_candidates = max(self.n_structures * 5, 50)
            ranked_list = transformation.apply_transformation(
                expanded_pmg, return_ranked_list=n_candidates
            )
            ...
            if len(ranked_list) <= self.n_structures:
                ...
                indices = np.linspace(
                    0, len(ranked_list) - 1, self.n_structures, dtype=int
                )
                ...
                if len(selected_entries) < self.n_structures:
                    remaining_indices = [
                        i for i in range(len(ranked_list)) if i not in indices
                    ]
                    for i in remaining_indices[
                        : self.n_structures - len(selected_entries)
                    ]:
