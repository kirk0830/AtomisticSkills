...
def main():
    ...
    parser.add_argument(
        "--input_dir", required=True, help="Directory containing input structures"
    )
    parser.add_argument(
        "--output_dir", required=True, help="Directory to save relaxed structures"
    )
    parser.add_argument(
        "--model", default="medium", help="MACE model size (small, medium, large)"
    )
    parser.add_argument(
        "--fmax", type=float, default=0.05, help="Force convergence criterion"
    )
