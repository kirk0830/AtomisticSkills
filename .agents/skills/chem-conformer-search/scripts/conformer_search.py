...
def smiles_to_atoms_list(
    smiles: str, num_conformers: int, rms_threshold: float
) -> List[Atoms]:
    """
    ...
    if not cids:
        logger.warning(
            "ETKDG failed to generate conformers. Trying with random coords..."
        )
    ...
    logger.info(
        f"RDKit generated {len(cids)} conformers after RMS pruning (threshold={rms_threshold} A)."
    )
...
def relax_conformers(
    atoms_list: List[Atoms], wrapper, fmax: float = 0.01
) -> List[Dict]:
    """
    ...
    logger.info(
        f"Relaxing {len(atoms_list)} conformers with {wrapper.model_name} (fmax={fmax})..."
    )
    ...
    for i, atoms in enumerate(atoms_list):
        ...
        try:
            ...
            relaxed_results.append({"atoms": atoms, "energy": energy, "original_id": i})
...
def align_and_calculate_rmsd(atoms1: Atoms, atoms2: Atoms) -> float:
    ...
    if (
        ...
        or atoms1.get_chemical_formula() != atoms2.get_chemical_formula()
    ):
        return 999.0
...
def deduplicate_or_cluster_results(
    results: List[Dict],
    method: str,
    ...
    if energy_threshold > 0:
        ...
        if before != len(results):
            logger.info(
                f"Energy pre-filter: {before} -> {len(results)} conformers (threshold {energy_threshold} eV above minimum)."
            )
    if len(results) == 1:
        return results
    ...
    if method == "rmsd":
        ...
        logger.info(
            f"Deduplicated {len(results)} -> {len(unique_results)} unique conformers (RMSD threshold {rmsd_threshold} A)."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate and rank molecular conformers."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Initial number of conformers (RDKit)",
    )
    parser.add_argument(
        "--rms_threshold", type=float, default=0.2, help="RDKit pruning RMSD threshold"
    )
    parser.add_argument(
        "--dedup_threshold",
        type=float,
        default=0.1,
        help="Post-relaxation RMSD threshold for deduplication (or hierarchical cut)",
    )
    parser.add_argument(
        "--clustering",
        type=str,
        default="rmsd",
        choices=["rmsd", "hierarchical", "kmeans"],
        help="Method to deduplicate/cluster conformers",
    )
    parser.add_argument(
        "--num_clusters",
        type=int,
        default=5,
        help="Number of clusters for KMeans option",
    )
    parser.add_argument(
        "--energy_threshold",
        type=float,
        default=0.5,
        help="Max energy above global min (eV) to keep before RMSD comparison. Set to 0 to disable.",
    )
    parser.add_argument(
        "--fmax", type=float, default=0.01, help="Relaxation fmax (eV/A)"
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=298.15,
        help="Temperature (K) for Boltzmann weighting",
    )
    ...
    parser.add_argument(
        "--model_name", default=None, help="Specific model name (e.g. MACE-OFF23-small)"
    )
    ...
    if args.smiles:
        initial_atoms = smiles_to_atoms_list(
            args.smiles, args.num_conformers, args.rms_threshold
        )
    else:
        initial_atoms = read_structure_file(args.structure)
        logger.info(
            f"Loaded {len(initial_atoms)} structures. Skipping RDKit generation."
        )
    ...
    unique_data = deduplicate_or_cluster_results(
        relaxed_data,
        args.clustering,
    ...
    logger.info("=" * 60)
    logger.info("Conformer search complete.")
