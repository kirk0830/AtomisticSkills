...
pixi run -e fairchem python .agents/skills/ml-fairchem-finetune/scripts/prepare_fairchem_data.py \
    --data private_data/WBM_subset_200_configs.json \
    --vasp-stress-conversion \
    --output-dir "$OUT_DIR"

echo "Generating FAIRCHEM Config..."
pixi run -e fairchem python .agents/skills/ml-fairchem-finetune/scripts/generate_fairchem_config.py \
...
conda run --live-stream -n fairchem fairchem -c uma_sm_finetune_template.yaml job.run_dir=runs/run_10ep +job.timestamp_id=run_10ep > train.log 2>&1
cd -
