...
conda run --live-stream -n fairchem python .agents/skills/ml-fairchem-finetune/scripts/prepare_fairchem_data.py \
...
conda run --live-stream -n fairchem fairchem -c uma_sm_finetune_template.yaml job.run_dir="../../runs" +job.timestamp_id=run_test

cd ../../../..

echo "Extracting training logs..."
conda run --live-stream -n fairchem python .agents/skills/ml-fairchem-finetune/scripts/extract_fairchem_logs.py \
