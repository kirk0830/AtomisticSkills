...
def test_train_cluster_expansion(skip_if_wrong_env, tmp_research_dir):
    ...
    res = smol_server.train_cluster_expansion(
        disordered_structure=struct, training_data=training_data, cutoffs={"2": 5.0}
    )
    assert (
        "error" in res
    )  # Because training data is empty, it shouldn't crash but will return error cleanly
...
def test_run_monte_carlo(skip_if_wrong_env, tmp_research_dir):
    ...
    res = smol_server.run_monte_carlo(
        supercell_matrix=[[2, 0, 0], [0, 2, 0], [0, 0, 2]], temperature=300, steps=100
    )
...
def test_fit_feature_matrix(skip_if_wrong_env):
    res = smol_server.fit_feature_matrix(
        feature_matrix_path="dummy.npy", energies_path="dummy.npy"
    )
    assert "error" in res
...
def test_check_mapping(skip_if_wrong_env):
    ...
    assert (
        "error" in res
    )  # Because CE isn't initialized or similar, but it tests function execution
