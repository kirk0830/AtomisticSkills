...
def test_explosion_monitor():
    ...
    assert (
        dyn.nsteps < 20
    ), f"Explosion monitor failed to stop simulation early (ran {dyn.nsteps}/20 steps)"

    # Verify the error message indicates explosion was detected
    assert (
        error_message and "Explosion detected" in error_message
...
def test_volume_monitor():
    ...
    assert (
        dyn.nsteps < 200
    ), f"Volume monitor failed to stop simulation early (ran {dyn.nsteps}/200 steps)"

    # Verify the error message indicates volume expansion was detected
    assert (
        error_message and "Volume expansion detected" in error_message
...
def test_equilibration_monitor():
    ...
    dyn = Langevin(
        atoms, timestep=2.0 * units.fs, temperature_K=temperature, friction=0.01
    )
    ...
    assert (
        dyn.nsteps < 200
    ), f"Equilibration monitor failed to stop simulation early (ran {dyn.nsteps}/200 steps)"

    # Verify the error message indicates equilibration was detected
    assert (
        error_message and "Equilibration reached" in error_message
