...
class TestSaveStructure:
    ...
    def test_saves_pymatgen_structure(
        self, sample_structure, tmp_path, skip_if_wrong_env
    ):
    ...
    def test_saves_to_various_formats(
        self, sample_structure, tmp_path, skip_if_wrong_env
    ):
...
class TestMaterialsProjectQueries:
    ...
    def test_get_structure_by_formula_returns_ase_atoms(self, skip_if_wrong_env):
        ...
        mock_mprester = Mock()
        mock_doc = Mock()
        mock_doc.structure = Structure.from_spacegroup(
        ...
        assert isinstance(result, Atoms), f"Expected ASE Atoms, got {type(result)}"
    ...
    def test_get_structure_by_chemsys_returns_most_stable(self, skip_if_wrong_env):
        ...
        stable_doc = Mock()
        stable_doc.structure = Structure.from_spacegroup(
            "Fm-3m", Lattice.cubic(5.43), ["Si"], [[0, 0, 0]]
        )
        stable_doc.energy_above_hull = 0.0

        unstable_doc = Mock()
        unstable_doc.structure = Structure.from_spacegroup(
            "Fd-3m", Lattice.cubic(5.5), ["Si"], [[0, 0, 0]]
        )
    ...
    def test_get_structure_by_id(self, skip_if_wrong_env):
        ...
        mock_mprester = Mock()
        mock_structure = Structure.from_spacegroup(
            "Fm-3m", Lattice.cubic(5.43), ["Si"], [[0, 0, 0]]
        )
        mock_mprester.materials.get_structure_by_material_id.return_value = (
            mock_structure
        )
