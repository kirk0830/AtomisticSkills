...
python .agent/skills/db-pdb/scripts/query_pdb.py \
  --search "HIV-1 protease" \
  --method "X-RAY DIFFRACTION" \
  --resolution 2.0 \
  --max_results 5 \
  --output .agent/skills/db-pdb/examples/hiv1_protease_search.json

# Step 2: Fetch metadata for a specific entry
python .agent/skills/db-pdb/scripts/query_pdb.py \
  --pdb_id 1HSG \
  --output .agent/skills/db-pdb/examples/1hsg_entry.json
```
