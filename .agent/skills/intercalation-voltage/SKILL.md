...
1.  **Prepare Structures**:
    ...
    ```bash
    # Env: base-agent
    python .agent/skills/intercalation-voltage/scripts/remove_atoms.py \
        LiFePO4.cif \
        --remove Li \
        --output FePO4.cif
    ```
...
3.  **Relax Structures Using MCP Tools**:
    ...
    # Relax bulk metal
    mcp_mace_relax_structure(
        structure_data=".agent/skills/intercalation-voltage/resources/Li_metal.cif",
...
5.  **Calculate Voltage**:
    ...
    ```bash
    # Env: base-agent
    python .agent/skills/intercalation-voltage/scripts/calculate_voltage.py \
        --e_full -123.45 \
        --e_empty -98.76 \
        --e_metal -1.23 \
        --n_metal 16 \
