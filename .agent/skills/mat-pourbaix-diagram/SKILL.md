...
```bash
# Env: base-agent
python .agent/skills/mat-pourbaix-diagram/scripts/calculate_pourbaix_mp.py \
    --comp_dict "Li=1,Fe=1" \
    --output ./output_dir
```
...
```bash
# Env: base-agent
python .agent/skills/mat-pourbaix-diagram/scripts/get_pourbaix_structures.py \
    --chemsys "Zn" \
...
Choose an appropriate MLIP based on your system (see `.agent/skills/ml-foundation-potentials/SKILL.md`).
...
```bash
# Env: base-agent
python .agent/skills/mat-pourbaix-diagram/scripts/calculate_pourbaix.py \
