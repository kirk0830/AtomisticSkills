...
```bash
# Env: base-agent
python .agent/skills/general-patent-search/scripts/query_google_patents.py "query string" --limit 10
```
...
```bash
# Env: base-agent
python .agent/skills/general-patent-search/scripts/query_google_patents.py "tetrafluoropropene OR HFO-1234yf" --limit 5
```

Searching for global patents assigned to SK Innovation regarding electrolytes:
```bash
# Env: base-agent
python .agent/skills/general-patent-search/scripts/query_google_patents.py "assignee:(SK Innovation) AND electrolyte" --limit 5
```
