...
4.  **Data Preparation**: Execute `scripts/prepare_mace_data.py` to convert JSON structures to `.xyz` and generate `finetune_config.yaml`.
5.  **Fine-Tuning**: Execute `mace_run_train --config /path/to/finetune_config.yaml` to begin fine-tuning natively on the GPU.
6.  **Validation**: Verify convergence and compare against the benchmarked foundation metrics.

## Training Configuration

MACE fine-tuning is divided into a data preparation step and a standard native training run. The script `scripts/prepare_mace_data.py` generates `.xyz` files and converts arguments into a fully-formed `finetune_config.yaml` configuration compatible with the MACE default parser.
...
### Model Freezing and Heads (Data Prep Script)
...
conda run -n base-agent python .agent/skills/ml-mace-finetune/scripts/extract_mace_logs.py \
