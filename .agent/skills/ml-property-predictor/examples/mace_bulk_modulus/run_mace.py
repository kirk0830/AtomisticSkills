...
def run_mace():
    ...
    script_path = os.path.join(base_dir, ".agent", "skills", "ml-property-predictor", "scripts", "train_mace_property.py")
...
if __name__ == "__main__":
    if "mace-agent" not in os.environ.get("CONDA_DEFAULT_ENV", ""):
        ...
        subprocess.run(["conda", "run", "-n", "mace-agent", "python", __file__], check=True)
    else:
        run_mace()
