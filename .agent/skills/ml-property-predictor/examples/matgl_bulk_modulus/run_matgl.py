...
def run_matgl():
    ...
    script_path = os.path.join(base_dir, ".agent", "skills", "ml-property-predictor", "scripts", "train_matgl_property.py")
    data_path = os.path.join(base_dir, ".agent", "test", "mp_bulk_modulus.json")
...
if __name__ == "__main__":
    if "matgl-agent" not in os.environ.get("CONDA_DEFAULT_ENV", ""):
        ...
        subprocess.run(["conda", "run", "-n", "matgl-agent", "python", __file__], check=True)
    else:
        run_matgl()
