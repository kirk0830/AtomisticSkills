...
Usage:
    ...
    python prepare_protein.py --pdb_id 1iep --assembly 1 --no_pdbqt --output_dir prep/

Requirements:
    - Conda environment: drugdisc-agent
    - Required packages: pdbfixer, openmm, meeko
    - mk_prepare_receptor.py available on PATH (installed with meeko)
"""
...
def run_meeko_prepare_receptor(
    pdb_in: Path,
    pdbqt_out: Path,
    meeko_json_out: Path,
    delete_residues: Optional[str] = None,
    default_altloc: Optional[str] = None,
    wanted_altloc: Optional[str] = None,
    log_path: Optional[Path] = None,
) -> Dict[str, Any]:
    """Generate receptor PDBQT using Meeko's mk_prepare_receptor.py."""
    exe = shutil.which("mk_prepare_receptor.py") or shutil.which("mk_prepare_receptor")
    if exe is None:
        raise RuntimeError(
            "mk_prepare_receptor.py not found on PATH. "
            "Install meeko and ensure the CLI scripts are available."
        )

    cmd = [
        exe,
        "--read_pdb", str(pdb_in),
        "--write_pdbqt", str(pdbqt_out),
        "--write_json", str(meeko_json_out),
    ]
    if delete_residues:
        cmd += ["--delete_residues", delete_residues]
    if default_altloc:
        cmd += ["--default_altloc", default_altloc]
    if wanted_altloc:
        cmd += ["--wanted_altloc", wanted_altloc]

    LOGGER.info("Running Meeko receptor prep: %s", " ".join(cmd))
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"mk_prepare_receptor failed (code {proc.returncode}). "
            f"See log: {log_path}"
        )

    return {
        "meeko_cmd": cmd,
        "meeko_log": str(log_path) if log_path else None,
        "pdbqt_file": str(pdbqt_out),
        "meeko_json": str(meeko_json_out),
    }
...
def prepare_protein(
    ...
    ph: float = 7.0,
    gen_pdbqt: bool = True,
    meeko_delete_residues: Optional[str] = None,
    meeko_default_altloc: Optional[str] = None,
    meeko_wanted_altloc: Optional[str] = None,
) -> Dict[str, Any]:
    ...
    if gen_pdbqt:
        pdbqt_out = output_dir / f"{name}_prepared.pdbqt"
        meeko_json = output_dir / f"{name}_prepared.meeko.json"
        meeko_log = output_dir / f"{name}_meeko.log"

        meeko_info = run_meeko_prepare_receptor(
            pdb_in=pdb_out,
            pdbqt_out=pdbqt_out,
            meeko_json_out=meeko_json,
            delete_residues=meeko_delete_residues,
            default_altloc=meeko_default_altloc,
            wanted_altloc=meeko_wanted_altloc,
            log_path=meeko_log,
        )
        summary.update(meeko_info)
...
def main() -> None:
    ...
    parser.add_argument(
        "--no_pdbqt", action="store_true",
        help="Skip PDBQT generation with Meeko",
    )
    parser.add_argument(
        "--meeko_delete_residues", default=None,
        help="Pass-through to Meeko --delete_residues selection language (e.g., 'A:350,B:15,16')",
    )
    parser.add_argument(
        "--meeko_default_altloc", default=None,
        help="Pass-through to Meeko --default_altloc (e.g., A)",
    )
    parser.add_argument(
        "--meeko_wanted_altloc", default=None,
        help="Pass-through to Meeko --wanted_altloc (e.g., ':5=B,B:17=A')",
    )
    ...
    summary = prepare_protein(
        ...
        meeko_delete_residues=args.meeko_delete_residues,
        meeko_default_altloc=args.meeko_default_altloc,
        meeko_wanted_altloc=args.meeko_wanted_altloc,
    )
    if url_used:
