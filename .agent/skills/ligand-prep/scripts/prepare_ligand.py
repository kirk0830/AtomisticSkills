...
class PrepResult:
    ...
    best_energy_kcal_mol: Optional[float] = None
    converged: Optional[bool] = None

    # outputs
    sdf_file: Optional[str] = None
    pdbqt_file: Optional[str] = None
    all_confs_sdf_file: Optional[str] = None
...
def prepare_one_state(
    ...
    if mol.GetNumConformers() == 0:
        conf_ids = embed_conformers(mol, num_confs=num_confs, prune_rms=prune_rms, seed=seed)
        if not conf_ids:
            res.error = "3D conformer embedding failed"
            return res
    else:
        conf_ids = [c.GetId() for c in mol.GetConformers()]

    res.num_conformers = int(mol.GetNumConformers())

    opt = optimize_conformers_mmff(mol, max_iters=max_iters, mmff_variant=mmff_variant)
    if opt is not None:
        res.force_field = f"MMFF ({mmff_variant})"
    else:
        opt = optimize_conformers_uff(mol, max_iters=max_iters)
        res.force_field = "UFF"
    if opt is None:
        res.error = "Force-field minimization failed (MMFF and UFF)"
        return res

    best_conf_id, best_energy, converged_any = select_best_conformer(mol, opt)
    res.best_energy_kcal_mol = float(round(best_energy, 6))
    res.converged = bool(converged_any)

    output_dir.mkdir(parents=True, exist_ok=True)
    sdf_path = output_dir / f"{safe_stem(name)}.sdf"
    write_best_sdf(mol, sdf_path, conf_id=best_conf_id, name=name, energy=res.best_energy_kcal_mol)
    res.sdf_file = str(sdf_path)

    if write_all_confs:
        all_path = output_dir / f"{safe_stem(name)}_allconfs.sdf"
        write_all_confs_sdf(mol, all_path, name=name, energies=opt)
        res.all_confs_sdf_file = str(all_path)

    if gen_pdbqt:
        pdbqt_path = output_dir / f"{safe_stem(name)}.pdbqt"
        try:
            best_mol = mol_with_single_conformer(mol, conf_id=best_conf_id)
            write_pdbqt_with_meeko(best_mol, pdbqt_path)
            res.pdbqt_file = str(pdbqt_path)
        except Exception as e:
            res.warnings.append(f"PDBQT generation failed: {e}")
...
def main() -> None:
    ...
    # conformers/minimization
    p.add_argument("--num_confs", type=int, default=20, help="Number of conformers to generate if none provided")
    p.add_argument("--prune_rms", type=float, default=0.5, help="RMS threshold (A) for pruning similar conformers")
    p.add_argument("--seed", type=int, default=42, help="Random seed for ETKDG embedding")
    p.add_argument("--max_iters", type=int, default=500, help="Max minimization iterations")
    p.add_argument("--mmff_variant", default="MMFF94", choices=["MMFF94", "MMFF94s"], help="MMFF variant")
    ...
    # outputs
    p.add_argument("--no_pdbqt", action="store_true", help="Skip PDBQT generation")
    p.add_argument("--write_all_confs", action="store_true", help="Write all conformers to <name>_allconfs.sdf")
        ...
        for pidx, psmi in enumerate(protomer_smiles):
            ...
            for tidx, tmol in enumerate(tautomers):
                ...
                r = prepare_one_state(
                    ...
                    write_all_confs=args.write_all_confs,
                    gen_pdbqt=(not args.no_pdbqt),
                )
