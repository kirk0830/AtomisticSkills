...
1.  **Background Research**: 
    ...
    - **MD Parameters**: Refer to the [mat-molecular-dynamics](../mat-molecular-dynamics/SKILL.md) skill for best practices on timesteps and monitors. In general, use a 2.0 fs timestep for systems without Hydrogen.

2.  **Phase Preparation**: 
    - **Solid**: Create a supercell using `create_supercell.py`.
    ```bash
    # Env: base-agent
    python .agent/skills/mat-melting-point/scripts/create_supercell.py [input_structure.cif] [solid_supercell.cif] --min_length 20.0
    ```
...
3.  **Interface Creation**: Use `create_interface.py` to concatenate the two phases.
    ```bash
    # Env: base-agent
    python .agent/skills/mat-melting-point/scripts/create_interface.py solid.cif liquid.cif --axis 0 --output interface.cif
    ```
...
5.  **Phase Verification**: Before running production MD, verify solid-liquid coexistence in all structures.
    ...
    python .agent/skills/mat-melting-point/scripts/check_phase.py solid_supercell.cif \
    ...
    python .agent/skills/mat-melting-point/scripts/check_phase.py liquid_supercell.cif \
        --solid_features <research_dir>/solid_features.json \
        --liquid_features <research_dir>/liquid_features.json
    
    # Interface should show ~50% solid/liquid coexistence
    python .agent/skills/mat-melting-point/scripts/check_phase.py interface_relax/relaxed_structure.cif \
        --solid_features <research_dir>/solid_features.json \
        --liquid_features <research_dir>/liquid_features.json
    ```
...
8.  **Phase Validation**: Verify that the solid and liquid phases still coexist at the end of the simulation.
    ...
    # Env: base-agent
    python .agent/skills/mat-melting-point/scripts/check_phase.py production_md/final_structure.cif
    ```
...
```bash
# Env: base-agent
python .agent/skills/mat-melting-point/scripts/create_interface.py Al_solid.cif Al_liquid.cif --axis 0 --output Al_interface.cif
```
