...
def run_atomate2_vasp_calculation(
    ...
    Args:
        ...
        calculation_type: "static", "relaxation", "band_structure", "optics", or "lobster".
    ...
    elif calculation_type in ["optics", "lobster"]:
        config = config or {}
    ...
    if execution_mode == "local":
        ...
        for i, flow in enumerate(flows):
            ...
            responses = run_locally(
                flow, create_folders=True, ensure_success=True, root_dir=str(struct_dir)
            )
            all_responses[str(flow.uuid)] = {
                "responses": responses,
                "dir": str(struct_dir),
            }
        ...
        try:
            ...
            job_id = handler.run_remote(
                parent_flow, project_name=project, worker_name=worker
            )
...
def get_atomate2_job_status(
    job_id: str, project_name: Optional[str] = None, save_to_file: Optional[str] = None
) -> Dict[str, Any]:
    """
