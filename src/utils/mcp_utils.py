    ...
    try:
        ...
        if custom_fd_str:
            try:
                ...
                sys.stderr.write(f"Warning: Failed to use MCP_STDOUT_FD={custom_fd_str}: {e}\n")
...
async def mcp_transport_manager(mcp_pipe_binary):
    ...
    async_stdin = anyio.wrap_file(TextIOWrapper(sys.stdin.buffer, encoding="utf-8", line_buffering=True))
    async_stdout = anyio.wrap_file(TextIOWrapper(mcp_pipe_binary, encoding="utf-8", line_buffering=True))
...
def run_fastmcp_server(mcp, mcp_pipe_binary):
    ...
    """
    async def _run():
        async with mcp_transport_manager(mcp_pipe_binary) as (read_stream, write_stream):
