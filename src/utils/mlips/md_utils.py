...
class EquilibrationMonitor:
    ...
    def _resolve_params(self, atoms, dyn=None, md_runner=None):
            ...
            if self.log_interval is None and hasattr(dyn, "observers"):
                for obs in dyn.observers:
                    try:
                        func = obs[0]
                        if func is self or (
                            hasattr(func, "__self__") and func.__self__ is self
                        ):
        ...
        if self.timestep_fs is None:
            raise ValueError(
                "timestep_fs must be provided or auto-detected from dynamics object."
            )
        if self.log_interval is None:
            raise ValueError(
                "log_interval must be provided to correctly evaluate simulation time."
            )
...
class VolumeMonitor:
    ...
    def __init__(
        self, atoms=None, lower_limit_ratio: float = 0.2, upper_limit_ratio: float = 2.0
    ):
        self.atoms = atoms
...
class DiffusionMonitor:
    ...
    def _resolve_params(self, atoms, dyn=None, md_runner=None):
        ...
        if self.log_interval is None:
            raise ValueError(
                "log_interval must be provided to evaluate diffusion correctly."
            )
        if self.temperature is None:
            raise ValueError(
                "temperature must be explicitly provided or auto-detected."
            )
        ...
        if self.check_interval_steps is None:
            ...
            self.check_interval_steps = max(
                1, int(self.check_interval_ps * 1000 / ts / li)
            )
    ...
    def finalize(self):
        ...
        if self.temperature is not None:
            ...
            raise ValueError(
                "temperature must be explicitly provided to DiffusionMonitor."
            )

        if self.timestep_fs is None:
            raise ValueError(
                "timestep_fs must be explicitly provided to DiffusionMonitor."
            )
        if self.log_interval is None:
            raise ValueError(
                "log_interval must be explicitly provided to DiffusionMonitor."
            )
        ...
        try:
            ...
            with open(
                os.path.join(self.output_dir, "diffusion_results.json"), "w"
            ) as f:
                import json
            ...
            plt.plot(
                ...
                y_fit,
                "r--",
                label=f"Fit (D={D:.2e} $\\pm$ {D_std:.2e} cm$^2$/s)",
            )
            ...
            plt.savefig(
                os.path.join(self.output_dir, f"msd_{self.specie}.png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
            ...
            logger.warning(
                f"Failed to export diffusion results during finalization: {e}"
            )
            ...
            if (n_total - self.ignore_entries) % self.check_interval_steps == 0:
                ...
                try:
                    ...
                    if D > 1e-8 and D_std > 0:
                        rel_err = D_std / D
                        logger.info(
                            f"Diffusion Monitor [{self.specie}]: D={D:.2e}, rel_err={rel_err:.3f} (target < {self.threshold}), mean_msd={mean_msd:.2f} (target >= {self.min_msd})"
                        )
                        ...
                        if rel_err < self.threshold and mean_msd >= self.min_msd:
                            ...
                            logger.info(
                                f"Diffusion Monitor [{self.specie}]: rel_err met ({rel_err:.3f} < {self.threshold}), but mean_msd ({mean_msd:.2f}) < {self.min_msd}"
                            )
                    else:
                        logger.info(
                            f"Diffusion Monitor [{self.specie}]: D={D:.2e}, D_std={D_std:.2e} (ignored due to insufficient diffusion or zero variance)"
                        )
...
class QuenchingControl:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if (
            hasattr(dyn, "sigma")
            and hasattr(dyn, "temp")
            and hasattr(dyn, "friction")
            and hasattr(dyn, "dt")
        ):
            import numpy as np
        ...
        if not hasattr(dyn, "set_temperature"):
            ...
            if thermo and hasattr(thermo, "_kT"):
                ...
                if (
                    hasattr(thermo, "_Q")
                    and hasattr(thermo, "_tdamp")
                    and hasattr(thermo, "_num_atoms_global")
                ):
                    # Q[0] scales with N_atoms, others scale with 1. Both scale with T.
                    thermo._Q[0] = (
                        3 * thermo._num_atoms_global * thermo._kT * thermo._tdamp**2
                    )
            ...
            if baro and hasattr(baro, "_kT"):
                ...
                if (
                    hasattr(baro, "_W")
                    and hasattr(baro, "_pdamp")
                    and hasattr(baro, "_num_atoms_global")
                ):
...
def get_md_callback(monitor_type: str, atoms=None, **kwargs):
    ...
    if monitor_type == "explosion":
        ...
        return EquilibrationMonitor(atoms=atoms, **kwargs)
