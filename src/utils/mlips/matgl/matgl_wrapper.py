...
if os.environ.get("MATGL_BACKEND") == "DGL" and MATGL_AVAILABLE:
    ...
    for k, v in AVAILABLE_MATGL_MODELS.items():
        if (
            "TensorNet" in k
            and not v.startswith("TensorNetDGL")
            and "TensorNet-MatPES" in v
        ):
    ...
    def apply_matgl_patches():
        ...
        if os.environ.get("MATGL_BACKEND") == "DGL":
            ...
            try:
                ...
                if hasattr(pes_dgl, "Potential") and not hasattr(
                    pes_dgl.Potential.forward, "__wrapped__"
                ):
                    pes_dgl.Potential.forward = ensure_dgl_device(
                        pes_dgl.Potential.forward
                    )
            except (ImportError, AttributeError):
                pass
            ...
            for path in ["matgl.utils.training", "matgl.utils._training_dgl"]:
                try:
                    ...
                    if hasattr(mod, "PotentialLightningModule"):
                        ...
                        if hasattr(cls, "step") and not hasattr(
                            cls.step, "__wrapped__"
                        ):
...
class MatGLWrapper(MLIPModel):
    ...
    def _load_model_agnostic(self, path_or_name: str) -> Any:
        ...
        init_args = torch.load(
            fpaths["model.pt"], map_location=map_location, weights_only=False
        )
    ...
    def load(self, model_path: Optional[str] = None) -> None:
        ...
        if Path(str(model_name_or_path)).is_file() and (
            str(model_name_or_path).endswith(".pth")
            or str(model_name_or_path).endswith(".pt")
        ):
            try:
                ...
                if isinstance(checkpoint, dict) and "model_name" in checkpoint:
                    ...
                    logger.info(
                        f"Detected checkpoint for base model: {base_model_name}"
                    )
                ...
                logger.warning(
                    f"Failed to load as checkpoint, falling back to standard load: {e}"
                )
    ...
    def static_calculation(self, structure_data: Any) -> Dict[str, Any]:
        ...
        if is_pes:
            ...
            if not hasattr(self.model, "calc_forces") and not hasattr(
                self.model, "model"
            ):
                is_pes = False
        ...
        if is_pes:
            ...
            try:
                ...
                if hasattr(self.model, "predict_structure"):
                    ...
                    if element_types:
                        converter = Structure2Graph(
                            element_types=element_types, cutoff=5.0
                        )
                        graph, _, _ = converter.get_graph(structure)
                        prediction = self.model(graph)
                    else:
                        raise AttributeError(
                            "Model has no 'predict_structure' and element_types are unknown."
                        )
                ...
                if (
                    "BandGap" in self.model_name
                    or "band_gap" in self.model_name.lower()
                ):
                    prop_name = "bandgap"
                elif (
                    "Eform" in self.model_name or "formation" in self.model_name.lower()
                ):
                ...
                return {
                    prop_name: val,
                    "unit": "eV"
                    if "BandGap" in self.model_name or "Eform" in self.model_name
                    else "unknown",
                }
            except Exception as e:
                import traceback
                ...
                logger.error(
                    f"Property prediction failed: {e}\n{traceback.format_exc()}"
                )
    ...
    def predict_atomic_features(self, structure_data: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create graph
            elements = (
                inner_model.element_types
                if hasattr(inner_model, "element_types")
                else get_element_list([struct])
            )
            ...
            state_attr = (
                torch.tensor(
                    state_attr, dtype=torch.float32, device=self.device
                ).unsqueeze(0)
            ...
            if "pos" not in g_batch.ndata:
                g_batch.ndata["pos"] = torch.tensor(
                    struct.cart_coords, dtype=torch.float32, device=self.device
                )

            # Handle PBC for distance calculations (DGL backend specific)
            if os.environ.get("MATGL_BACKEND") == "DGL":
                lattice = torch.tensor(
                    struct.lattice.matrix, dtype=torch.float32, device=self.device
                ).unsqueeze(0)
                g_batch.edata["lattice"] = torch.repeat_interleave(
                    lattice, g_batch.batch_num_edges(), dim=0
                )
                g_batch.edata["pbc_offshift"] = (
                    g_batch.edata["pbc_offset"].unsqueeze(dim=-1)
                    * g_batch.edata["lattice"]
                ).sum(dim=1)

            # Run model with return_all_layer_output=True to get latent features
            with torch.no_grad():
                model_output = inner_model(
                    g=g_batch,
            ...
            if not isinstance(model_output, dict):
                return {
                    "error": f"Model of type {type(inner_model)} did not return a dict when return_all_layer_output=True."
                }
            ...
            if features is None:
                return {
                    "error": f"Could not find node features in model output. Available keys: {list(model_output.keys())}"
                }
            ...
            logger.error(
                f"Failed to predict atomic features (MatGL): {e}\n{traceback.format_exc()}"
            )
    ...
    def get_supported_elements(self) -> List[str]:
        ...
        if self.is_loaded:
            ...
            if hasattr(potential, "model"):
                ...
                # Check for AtomRef
                if (
                    hasattr(inner_model, "atom_ref")
                    and inner_model.atom_ref is not None
                ):
