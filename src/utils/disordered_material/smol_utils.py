...
try:
    ...
    _original_get_species = getattr(
        smol_domain, "_original_get_species", smol_domain.get_species
    )
    _original_get_el_sp = getattr(
        smol_domain, "_original_get_el_sp", smol_domain.get_el_sp
    )
...
class SmolWrapper:
    ...
    def create_subspace(
        ...
        if isinstance(disordered_structure, (str, bytes, os.PathLike)):
            ...
            if not loaded:
                raise ValueError(
                    f"Could not load structure from {disordered_structure}"
                )
            struct = loaded[0]
        else:
        ...
        self.subspace = ClusterSubspace.from_cutoffs(
            struct, cutoffs=cutoffs, basis=basis_set
        )
    ...
    def add_training_data(
        ...
        if isinstance(data, (str, os.PathLike)):
            ...
            if os.path.isfile(path):
                ...
                cif_files = glob.glob(
                    os.path.join(path, "**", "relaxed_structure.cif"), recursive=True
                )
                for cif in cif_files:
                    try:
                        ...
                        if os.path.exists(energy_file):
                            ...
                            entry = {"structure": struct, "energy": energy, "path": cif}
                ...
                if not data_list:
                    ...
                    logger.info(
                        "No standard result files types 1/2 found. Trying generic load_structures."
                    )
                    try:
                        ...
                        for s in structs:
                            ...
                            if hasattr(s, "entry") and hasattr(s.entry, "energy"):
                                e = s.entry.energy
                            elif hasattr(s, "properties") and s.properties.get(
                                "energy"
                            ):
    ...
    def fit_expansion(self, method: str = "ls", **kwargs) -> Dict[str, Any]:
        ...
        if method == "sgl":
            ...
            lasso_cv = LassoCV(fit_intercept=True).fit(
                feature_matrix[:, point_cluster_start:point_cluster_end],
                energies.ravel() - e0,
            )
            e1 = lasso_cv.predict(
                feature_matrix[:, point_cluster_start:point_cluster_end]
            ...
            eci = np.concatenate(
                ([e0 + lasso_cv.intercept_], lasso_cv.coef_, sgl.coef_), axis=0
            )
            ...
            y_pred = (
                e0
                + e1.ravel()
                + np.dot(feature_matrix[:, point_cluster_end:], sgl.coef_)
            )
        else:
            if method == "lasso":
                model = Lasso(**kwargs)
        ...
        return {
            ...
            "rmse": rmse,
            "loocv": loocv,
            "coef_count": len(np.where(np.abs(eci) > 1e-5)[0])
            if method == "sgl"
            else len(eci),
        }
    ...
    def fit_feature_matrix(
        ...
        if fit_method == "sgl":
            ...
            if len(groups) == X.shape[1]:
                ...
                return {
                    "error": f"Size of groups ({len(groups)}) doesn't match X features ({X.shape[1]}) minus point_features_count ({non_grouped_cols})."
                }
    ...
    def sample_ordered_structures(
        ...
        dummy_ce = ClusterExpansion(
            self.subspace, np.zeros(self.subspace.num_corr_functions)
        )
        ensemble = Ensemble.from_cluster_expansion(
            dummy_ce, np.eye(3, dtype=int), processor_type="expansion"
        )

        site_spaces = [
            list(sl.site_space.keys()) for sl in ensemble.sublattices if sl.is_active
        ]
        sublattice_sizes = [
            len(sl.sites) for sl in ensemble.sublattices if sl.is_active
        ]
        ...
        selected_structs, _, _ = generate_training_structures(
            self.subspace, sc_matrices, comp_space, num_structs=num_structures
        )
    ...
    def run_mc(
        ...
        # Create Ensemble
        ensemble = Ensemble.from_cluster_expansion(
            self.expansion, supercell_matrix=supercell_matrix
        )
        ...
        sampler = Sampler.from_ensemble(
            ensemble, temperature=temperature, step_type=step_type
        )
            ...
            if initial_composition is not None:
                try:
                    ...
                    if isinstance(initial_composition, dict):
                        ...
                        for sl in sublattices:
                            ...
                            for sp in sl.site_space:
                                ...
                                if sp_name in initial_composition:
                                    ...
                                    sl_dict[sp] = initial_composition[
                                        sp_name.replace("A0+", "")
                                    ]

                            if not sl_dict:
                                sl_dict = {
                                    sp: 1.0 / len(sl.species) for sp in sl.species
                                }
                        ...
                        initial_comp_obj = [
                            Composition({get_species(k): v for k, v in d.items()})
                            for d in initial_composition
                        ]
                    else:
                        initial_comp_obj = initial_composition

                    logger.info(
                        f"Generating random initial state with composition: {initial_comp_obj}"
                    )
                    initial_occupancies = generate_random_ordered_occupancy(
                        ensemble.processor, composition=initial_comp_obj
                    )
    ...
    def compute_feature_vectors(
        self, structures: List[Union[Structure, Dict, str]]
    ) -> Dict[str, Any]:
        """
        ...
        return {"features": features, "valid_indices": valid_indices, "errors": errors}
    ...
    def _occu_to_structure(
        self, ensemble: Ensemble, occupancies: np.ndarray
    ) -> Structure:
        ...
        for i, site in enumerate(structure):
            if isinstance(site.species, Composition) and not site.species.is_element:
                ...
                best_sp = sorted(
                    site.species.items(), key=lambda x: x[1], reverse=True
                )[0][0]
                structure.replace(i, best_sp)

        return structure

    def sweep_cutoffs_and_train(
        self,
            ...
            for c3 in cutoff_ranges.get(3, [0]):
                for c4 in cutoff_ranges.get(4, [0]):
                    ...
                    try:
                        ...
                        mse = res["rmse"] ** 2
                        bic = n_entries * np.log(max(mse, 1e-10)) + n_params * np.log(
                            n_entries
                        )
        ...
        if not best_results:
            return {
                "error": "No valid model found during sweep. Try more data or smaller cutoffs."
            }
