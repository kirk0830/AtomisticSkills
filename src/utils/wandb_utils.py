...
def log_training_metrics(
    epoch: int, metrics: Dict[str, float], prefix: str = ""
) -> None:
    """
...
def log_training_history(
    ...
    for epoch in range(max_epochs):
        ...
        if "energy_mae_train" in training_history and epoch < len(
            training_history["energy_mae_train"]
        ):
        ...
        if "energy_mae_val" in training_history and epoch < len(
            training_history["energy_mae_val"]
        ):
        ...
        if "force_mae_train" in training_history and epoch < len(
            training_history["force_mae_train"]
        ):
        ...
        if "force_mae_val" in training_history and epoch < len(
            training_history["force_mae_val"]
        ):
        ...
        # Stress MAE
        if "stress_mae_train" in training_history and epoch < len(
            training_history["stress_mae_train"]
        ):
            val = training_history["stress_mae_train"][epoch]
            if val is not None:
                metrics["train/stress_mae"] = val

        if "stress_mae_val" in training_history and epoch < len(
            training_history["stress_mae_val"]
        ):
        ...
        # Loss
        if "loss_train" in training_history and epoch < len(
            training_history["loss_train"]
        ):
    ...
    if (
        "energy_distribution" in training_history
        and training_history["energy_distribution"]
    ):
        wandb.log(
            {
                "data/energy_distribution": wandb.Histogram(
                    training_history["energy_distribution"]
                )
            }
        )

    if (
        "force_distribution" in training_history
        and training_history["force_distribution"]
    ):
        wandb.log(
            {
                "data/force_distribution": wandb.Histogram(
                    training_history["force_distribution"]
                )
            }
        )

    if (
        "stress_distribution" in training_history
        and training_history["stress_distribution"]
    ):
        wandb.log(
            {
                "data/stress_distribution": wandb.Histogram(
                    training_history["stress_distribution"]
                )
            }
        )
...
def parse_wandb_config_from_query(query: str) -> Dict[str, Any]:
    ...
    config = {"project": None, "create_new": None, "tags": []}

    # Check for new project keywords
    if any(
        keyword in query_lower
        for keyword in ["new project", "create project", "new wandb project"]
    ):
