...
class Atomate2Handler:
    ...
    def check_environment(self) -> Dict[str, Any]:
        ...
        checks = {"atomate2": False, "vasp": False, "potcar": False, "error": None}
    ...
    def get_project_name(self, project_name: Optional[str] = None) -> str:
        ...
        try:
            from jobflow_remote.config.manager import (
                ConfigManager,
            ...
            if project_name:
                return project_name
            raise ValueError(
                "jobflow-remote not found. Please provide project_name explicitly."
            )
        ...
        try:
            ...
            if env_project:
                try:
                    ...
                    logger.warning(
                        f"ATOMATE2_REMOTE_PROJECT='{env_project}' is set but invalid/unknown."
                    )
    ...
    def get_flow_maker(
        ...
        from atomate2.vasp.jobs.matpes import (
        ...
        if preset == "mp-r2scan":
            ...
            if calculation_type == "relaxation":
                ...
                raise ValueError(
                    f"Unsupported calculation_type {calculation_type} for mp-r2scan"
                )
            if user_incar:
            ...
            return MatPesStaticFlowMaker(static1=None, static2=meta_maker, static3=None)
        elif preset == "matpes-pbe":
            gga_maker = MatPesGGAStaticMaker()
            if user_incar:
                gga_maker.input_set_generator.user_incar_settings.update(user_incar)
            return MatPesStaticFlowMaker(static1=gga_maker, static2=None, static3=None)
            ...
            if user_incar:
                if calculation_type == "band_structure":
                    ...
                    if hasattr(maker.bs_maker, "input_set_generator"):
                        maker.bs_maker.input_set_generator.user_incar_settings.update(
                            user_incar
                        )
                elif calculation_type == "optics":
                    if hasattr(maker.static_maker, "input_set_generator"):
                    ...
                    if hasattr(maker.relax_maker, "input_set_generator"):
                        maker.relax_maker.input_set_generator.user_incar_settings.update(
                            user_incar
                        )
                    if hasattr(maker.lobster_static_maker, "input_set_generator"):
                        maker.lobster_static_maker.input_set_generator.user_incar_settings.update(
                            user_incar
                        )
                else:
                    maker.input_set_generator.user_incar_settings.update(user_incar)
    ...
    def check_status(
        self, job_id: str, project_name: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if responses_file.exists():
            ...
            if status_info.get("remote"):
                try:
                    ...
                    if not results:
                        return {
                            "error": "Remote results not found or not yet available."
                        }
        ...
        for flow_uuid, data in all_responses.items():
            ...
            for path in valid_paths:
                ...
                entry = {
                    ...
                    "energy": getattr(
                        output, "energy", getattr(output, "final_energy", None)
                    ),
                ...
                if entry["stress"] is not None:
                    ...
                    entry["stress"] = (
                        np.array(entry["stress"]) * -1.0 * ase.units.GPa
                    ).tolist()
    ...
    def get_database_summary(
        ...
        try:
            ...
            logger.error(
                f"Failed to connect to JobController project '{project_name}': {e}"
            )
            return {"error": str(e)}
                ...
                results.append(
                    {
                        ...
                        "name": j.name,
                        "state": j.state.name,
                        "completed_at": j.updated_on.isoformat()
                        if j.updated_on
                        else None,
                    }
                )
    ...
    def _get_training_data_core(
        ...
        if job_ids:
            ...
            if uuids:
                try:
                    ...
                    f_ids = list(
                        set([j.flow_id for j in jobs_info if hasattr(j, "flow_id")])
                    )
                    if f_ids:
        ...
        if not (job_ids or flow_ids) and (formula or chemsys):
            try:
                ...
                docs_to_process = list(
                    jc.jobstore.docs_store._collection.find(query)
                    .sort("output.last_updated", -1)
                    .limit(limit)
                )
            except Exception as e:
            ...
            for flow_info in all_flows:
                for jid in getattr(flow_info, "job_ids", []):
                    ...
                    try:
                        ...
                        if not doc:
                            doc = jobstore.get_output(
                                getattr(flow_info, "flow_id", jid)
                            )
                        if doc:
        ...
        for doc in docs_to_process:
            ...
            # Energy
            energy = (
                actual_output.get("energy")
                or actual_output.get("final_energy")
                or task_doc.get("energy")
            )
